...
usage = 'python build_post_trans_kinetic_model.py [1] [2]\n[1] control file path\n[2] PDBID'
if len(sys.argv) != 3:
    print(usage)
    sys.exit()
...
def gen_state_visualizion(state_id, psf, native_cor, state_cor, native_AA_pdb, if_entangled):
    ...
    # backmap
    os.system('backmap_pulchra_only.py -i '+native_AA_pdb+' -c tmp.pdb')
...
for i_ax, mutant_type in enumerate(mutant_type_list):
    ...
    # Get Qbb trajectory
    print(f'Get: '+OP_dir+mutant_type+'_QBB'+prefix_file+'.npy')
    qbb_list = list(np.load(OP_dir+mutant_type+'_QBB'+prefix_file+'.npy', allow_pickle=True))
    qbb_list = [q.reshape(q.shape[0],1) for q in qbb_list]

    # Get Entanglement trajectory
    print('Get: '+OP_dir+mutant_type+'_Entanglement.npy')
    G_list_0 = list(np.load(OP_dir+mutant_type+'_Entanglement.npy', allow_pickle=True))
...
for i, meta_state in enumerate(samples):
    ...
    for idx in meta_state:
        ...
        traj_file_path = po_dir+str(traj_idx_1+1)+'/'+str(traj_idx_1+1)+f'_i33_post_trans_{pdb}.dcd'
